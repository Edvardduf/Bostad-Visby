...
const ListingForm = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "renterEmail") {
      ...
      setTimeout(() => checkRenterExistence(value), 300);
    }
  };

  const checkRenterExistence = async (email) => {
    try {
        const response = await fetch(`http://localhost:8000/renter/${encodeURIComponent(email)}`);
...
const debounceTimeoutRef = useRef();
  ...
  return (
    <div className=" ml-4 rounded-lg shadow-lg max-w-3xl mx-auto my-8"
    style={{
        background:
    ...
    onBlur={() => debouncedCheckRenterExistence(formData.renterEmail)}
